...
func TestPasetoV3Public(t *testing.T) {
	...
	t.Run("NewPasetoV3Public", func(t *testing.T) {
		maker, err := NewPasetoV3Public(privateKeyHex, publicKeyHex)
		require.NoError(t, err)
		require.NotNil(t, maker)

		// Test invalid private key
		_, err = NewPasetoV3Public("invalid_private_key", publicKeyHex)
		require.Error(t, err)
		require.Contains(t, err.Error(), "could not initialize private asymmetric key")

		// Test invalid public key
		_, err = NewPasetoV3Public(privateKeyHex, "invalid_public_key")
		require.Error(t, err)
	...
	t.Run("WrongPublicKey", func(t *testing.T) {
		...
		// Create new key pair
		differentPrivateKey := paseto.NewV3AsymmetricSecretKey()
		differentPublicKey := differentPrivateKey.Public()

		// Create new maker with different public key
		wrongMaker, err := NewPasetoV3Public(privateKeyHex, differentPublicKey.ExportHex())
		require.NoError(t, err)
